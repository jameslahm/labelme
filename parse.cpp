...
void My::Parse::parse(const QString& fileName,QList<My::Shape*>& shapes,const QString& labelfileName){
...
//                brushShape->radius.append(dataMap.value("radius").toFloat());
            shape=brushShape;
            shape->label=label;
            shape->color=color;
        }
...
void My::Parse::write(const QString& fileName,QList<My::Shape*>& shapes){
    QFileInfo fileInfo(fileName);
...
//                foreach(cv::Point3f point3d,brush3d->points){
//                    if(int(point3d.z)==labels[0]->zindex){
//                        QPointF p(point3d.x,point3d.y);
//                        brush->points.append(p);
//                    }
        ...
        for(int i=0;i<shapes.length();i++){
            if(shapes[i]->type==My::Shape3D::Rectangle3D){
                ...
                if(labels[2]->zindex>=(rectangle3d->points[0].x*list[0].cols) && (labels[2]->zindex<=(rectangle3d->points[0].x+rectangle3d->width)*list[0].cols)){
                    ...
                    rectangle->height=float(rectangle3d->zspan)/float(list.length());
//                    rectangle->points.append(QPointF(1-(rectangle3d->points[0].y+rectangle3d->height),rectangle3d->points[0].z/list.length()));
